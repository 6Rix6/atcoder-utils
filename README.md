<div align="center">

![logo](./images/logo.png)

<h1><b>AtCoder Utils</b></h1>

</div>

AtCoder で問題を解く際に便利な機能を提供する拡張機能です。

## Cookie の設定方法

> [!IMPORTANT]
> コンテスト中の問題を取得するには AtCoder の Cookie が必要です。過去問のみの使用であれば設定は不要です。

1. ブラウザで AtCoder を開きログインする
2. F12 もしくは 右クリック>検証 で開発者ツールを開き、`Application`タブ > `Storage`> `Cookie`> `https://atcoder.jp` を選択。
3. `REVEL_SESSION` の値をコピーして、コマンドパレットから`AtCoder Utils: Set Cookie`を実行し、貼り付ける

## スクリーンショット

![screenshot](./images/atcoder-panel.png)

## 実行モード

コードの実行方法を **Paiza.io（オンライン）** と **ローカル実行** から選べます。

VS Code の設定メニューから 拡張機能 > AtCoder Utils > **Execution Mode** で切り替えてください。

| モード                | 説明                                                                |
| --------------------- | ------------------------------------------------------------------- |
| `paiza`（デフォルト） | Paiza.io API を使用してオンラインで実行（インターネット接続が必要） |
| `local`               | ローカルにインストールされたコンパイラ/インタプリタで実行           |

ローカル実行の対応言語やカスタムコマンドの設定方法については [LOCAL_EXECUTION.md](./LOCAL_EXECUTION.md) を参照してください。

## 対応言語（Paiza.io）

`Python 3` `Python 2` `JavaScript` `TypeScript` `C` `C++` `C#` `Java` `Kotlin` `Scala` `Swift` `Go` `Rust` `Ruby` `PHP` `Perl` `Bash` `R` `Haskell` `Erlang` `Elixir` `Clojure` `F#` `Visual Basic` `COBOL` `D` `Scheme` `Common Lisp` `CoffeeScript` `Objective-C` `MySQL` `なでしこ` `Brainfuck`

## 必要条件

- VS Code 1.104.0 以上
- インターネット接続（Paiza.io モード使用時）

## ライセンス

[MIT](./LICENSE)
