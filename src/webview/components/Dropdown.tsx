...
const ListboxElm = styled.div<{ position: string }>`
  ...
  border-radius: ${(props) =>
    props.position === "above" ? "2px 2px 0 0" : "0 0 2px 2px"};
...
const Dropdown = ({
  ...
  return (
    <SelectElm
      className={`vscrui-dropdown ${isDisabled ? "disabled" : ""} ${
        className || ""
      }`}
      onKeyDown={onKeyDown}
      ref={dropdownRef}
    ...
    >
      ...
      {isOpen && !isDisabled && (
        <ListboxElm className={`vscrui-checkbox__listbox`} position={position}>
          <ul>
            {options.map((option, index) => {
              ...
              return (
                <li key={index} onMouseEnter={() => setActiveIndex(index)}>
                  <ListItemElm
                    key={index}
                    className={`vscrui-checkbox__listbox__item ${
                      (activeIndex === null && selectedValue === value) ||
                      activeIndex === index
                        ? "active"
                        : ""
                    }`}
